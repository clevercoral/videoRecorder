...
saveVedioThread::saveVedioThread()
{
    m_video=new PicInPic_Read;
    m_audio=new Audio_Read;
    connect(m_video,SIGNAL(SIG_sendVideoFrame(QImage)),this,SIGNAL(SIG_sendVideoFrame(QImage)));
    connect(m_video,SIGNAL(SIG_sendPicInPic(QImage)),this,SIGNAL(SIG_sendPicInPic(QImage)));
    connect(m_video,SIGNAL(SIG_sendVideoFrameData( uint8_t*,int)),this,SLOT(slot_getVideoFrameData(uint8_t*,int)));
    connect(m_audio,SIGNAL(SIG_sendAudioFrameData( uint8_t*, int)),this,SLOT(slot_getAudioFrameData(uint8_t*,int)));
}
int saveVedioThread::write_frame(AVFormatContext *fmt_ctx, AVCodecContext *c,
...
void saveVedioThread::slot_getVideoFrameData(uint8_t *picture_buf, int buffer_size)
{
    //记录第一次时间
...
void saveVedioThread::slot_getAudioFrameData(uint8_t *picture_buf, int buffer_size)
{
    //视频如果没有 , 丢弃帧 拉齐视音频时间
    if( (m_avFormat.hasCamera || m_avFormat.hasDesktop)&& m_videoBeginFlag )
    {
        free( picture_buf );
        return;
    }
...
//这里将音频和视频写在一起了，
void saveVedioThread::slot_openVideo()
{
    //打开数据采集
    if(m_avFormat.hasCamera|m_avFormat.hasDesktop)
    {
...
void saveVedioThread::close_stream(AVFormatContext *oc, OutputStream *ost)
{
...
void saveVedioThread::slot_closeVideo()
{
//    //关闭数据采集
//    if(m_avFormat.hasCamera|m_avFormat.hasDesktop)
...
//    avformat_free_context(oc);
    //由于数据采集的关闭和相关缓冲区的回收在run函数做了，（由于是两个线程，如果在这里做，可能会导致回收问题)
    if( m_avFormat.hasCamera || m_avFormat.hasDesktop)
        m_video->slot_closeVideo();
     if( m_avFormat.hasAudio )
        m_audio->slot_closeAudio();
...
void saveVedioThread::add_video_stream(OutputStream *ost, AVFormatContext *oc,
                                       AVCodec **codec,
...
void saveVedioThread::open_video(AVFormatContext *oc, AVCodec *codec, OutputStream *ost)
{
    int ret;
...
void saveVedioThread::add_audio_stream(OutputStream *ost, AVFormatContext *oc,
                                       AVCodec **codec,
                                       enum AVCodecID codec_id)
{
    AVCodecContext *c;
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    c->sample_fmt  = AV_SAMPLE_FMT_FLTP;//采样格式
    c->bit_rate    = 64000;
    c->sample_rate = 44100;
...
void saveVedioThread::open_audio(AVFormatContext *oc, AVCodec *codec, OutputStream
                                 *ost )
{
    AVCodecContext *c;
    int nb_samples;
...
void saveVedioThread::slot_setInfo(STRU_AV_FORMAT &avFormat)
{
    m_avFormat=avFormat;
    char filename[260]="";
    sprintf(filename,"%s",m_avFormat.fileUrl.toStdString().c_str());
    //编码的准备，粘贴ffmpeg中的编码方式，做好准备工作

    //这里的封装格式改为flv，准为将来推流做准备
    ...
    if (m_avFormat.hasCamera||m_avFormat.hasDesktop) {
        add_video_stream(&video_st, oc, &video_codec, AV_CODEC_ID_H264);//使用h264编码
        have_video = 1;
        encode_video = 1;
    }
    if (m_avFormat.hasAudio) {
        add_audio_stream(&audio_st, oc, &audio_codec, AV_CODEC_ID_AAC);//使用aac编码器
        have_audio = 1;
        encode_audio = 1;
    }
...
void saveVedioThread::openWhiteFace()
{
    m_video->startWhiteFace();
}


void saveVedioThread::videoDataQuene_Input(uint8_t * buffer, int size, int64_t
                                           time)
{
...
BufferDataNode *saveVedioThread::videoDataQuene_get(int64_t time)
{
...
void saveVedioThread::audioDataQuene_Input(const uint8_t *buffer, const int &size)
{
    BufferDataNode *node = (BufferDataNode*)malloc(sizeof(BufferDataNode));
    node->bufferSize = size;
...
BufferDataNode * saveVedioThread::audioDataQuene_get()
{
    BufferDataNode *node = NULL;
    m_audioMutex.lock();
...
bool saveVedioThread::write_video_frame(AVFormatContext *oc, OutputStream *ost,
                                       double time)
{
...
int saveVedioThread::write_frame(AVFormatContext *fmt_ctx, AVCodecContext
...
bool saveVedioThread::write_audio_frame(AVFormatContext *oc, OutputStream *ost)
{
    AVCodecContext *c;
    c = audio_st.enc;
    //从队列里面获取
